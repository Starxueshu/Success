...
#应用标题
st.title('A machine learning-based predictive model for predicting leakage of Percutaneous vertebroplasty')
...
thyroid_train['Leakage'] = thyroid_train['Leakage'].apply(lambda x : +1 if x==1 else 0)
#thyroid_test = pd.read_csv('test.csv', low_memory=False)
#thyroid_test['Leakage'] = thyroid_test['Leakage'].apply(lambda x : +1 if x==1 else 0)
features = ['Age','high','BMI','BMD','hospitalization.time','surgery.time','Injury.to.surgery','Multiple']
#全部变量
...
sp = 0.5
#figure
is_t = (RF.predict_proba(np.array([[Age,high,BMI,BMD,hospitalization_time,surgery_time,Injury_to_surgery,Multiple]]))[0][1])> sp
prob = (RF.predict_proba(np.array([[Age,high,BMI,BMD,hospitalization_time,surgery_time,Injury_to_surgery,Multiple]]))[0][1])*1000//1/10
...
if st.button('Predict'):
    st.markdown('## Risk grouping for Leakage:  '+str(result))
    if result == 'Low Risk':
        st.balloons()
    st.markdown('## Probability of Leakage:  '+str(prob)+'%')
